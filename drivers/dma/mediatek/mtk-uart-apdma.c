...
static const struct of_device_id mtk_uart_apdma_match[] = {
	...
	{ .compatible = "mediatek,mt6779-uart-dma", .data = (void *)34 },
...
static int mtk_uart_apdma_probe(struct platform_device *pdev)
{
	...
	bits = (uintptr_t)of_device_get_match_data(&pdev->dev);
	mtkd->support_extended_addr = bits > 32;
	rc = dma_set_mask_and_coherent(&pdev->dev, DMA_BIT_MASK(bits));
	if (rc)
		return rc;
